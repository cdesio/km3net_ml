...
folder = IRON_HIDE #UNISA
main_output_dir = os.path.join(folder, 'train_test_files')
if not os.path.exists(main_output_dir):
    os.makedirs(main_output_dir)

output_dir = os.path.join(main_output_dir, 'numu_nue_stratified_labels')
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

fnames_numu, fnames_nue = get_multi_data_files(folder, merge=False)
...
for i in range(100):
    export_train_validation_test(i, fnames_numu[i], fnames_nue[i], out_dir=output_dir, target_key='y', stratify_key='y')
